...
<head>
    ...
    <title>@yield('title', 'Cashier') - Logos Coffe</title>
...
<body class="bg-gray-50 flex min-h-screen">
    ...
    <aside class="w-64 bg-black text-white p-6 flex flex-col fixed h-full">
        <div class="mb-10 text-center">
            ...
            <h2 class="font-heading text-lg font-bold">Cashier Panel</h2>
        </div>

        <nav class="flex-1 space-y-2">
            <a href="/cashier"
                class="block px-4 py-2 {{ request()->is('cashier') ? 'bg-gray-900' : 'hover:bg-white/10' }} rounded-lg text-sm font-medium transition-colors">Dashboard</a>
            ...
            <a href="/cashier/orders"
                class="block px-4 py-2 {{ request()->is('cashier/orders*') ? 'bg-gray-900' : 'hover:bg-white/10' }} rounded-lg text-sm font-medium transition-colors">Pesanan</a>
            <a href="/cashier/products"
                class="block px-4 py-2 {{ request()->is('cashier/products*') ? 'bg-gray-900' : 'hover:bg-white/10' }} rounded-lg text-sm font-medium transition-colors">Stok
                Produk</a>
            <a href="/cashier/reports/sales"
                class="block px-4 py-2 {{ request()->is('cashier/reports*') ? 'bg-gray-900' : 'hover:bg-white/10' }} rounded-lg text-sm font-medium transition-colors">Rekap
                Penjualan</a>
        </nav>
